...
export function buildOnlineReplySystemPrompt({ playbook }) {
  ...
  return [
    ...
    "称呼纪律：有评论者昵称时，回复中须带上其昵称（通常放开头）；问候、致歉等开场措辞按评论语境与原文语言灵活组织，勿套用固定外语模板。无昵称时用中性礼貌开场。禁止兄弟/bang/bro/亲/老铁等俚语或泛称。",
    playbook,
...
export function buildOnlineReplyUserPrompt({ author, rating, hints, content }) {
  ...
  const salutationHint = authorName
    ? `称呼：评论者昵称「${authorName}」，回复中须带上；问候、致歉等开场按评论语言与语境灵活写，勿 bang/bro/兄弟等。`
    : "称呼：无评论者昵称，用与评论语言一致的中性礼貌开场，勿用兄弟/亲等。";
  return [
    `场景：${hints.scenario}`,
    `评分：${rating} 星`,
