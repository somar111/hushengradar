...
export function filterIncomingSubcategories({
  ...
  for (const raw of incoming ?? []) {
    ...
    let label = normalizeSubcategoryLabel(raw.label);
...
export function mergeSubcategoriesGuarded(existing = [], incoming = [], opts) {
  ...
  for (const s of [...existing, ...accepted]) {
    ...
    if (!byKey.has(key)) byKey.set(key, { key, label: normalizeSubcategoryLabel(s.label) });
  }
  return [...byKey.values()];
}
...
export function applyGuardedSubMerge(baseSubs, proposed, opts) {
  ...
  for (const p of proposed ?? []) {
    ...
    byKey.set(k, { key: k, label: normalizeSubcategoryLabel(p.label) });
  }
  return [...byKey.values()];
}

/** bootstrap / 修订 AI 返回整份 taxonomy 时：各父类内去 sibling 近义 */
