...
export function sanitizeTaxonomy(rawCategories) {
  ...
  for (const c of rawCategories) {
    ...
    for (const s of c.subcategories ?? []) {
      ...
      subSeen.add(sk);
      subcategories.push({ key: sk, label: normalizeSubcategoryLabel(s.label) });
    }
    out.push({
      key,
  ...
  return guardTaxonomySubcategories(out);
}

/** taxonomy subLabel 变更后，生成确定性 remap 以同步历史 ai_tags */
    ...
    for (const s of c.subcategories ?? []) {
      ...
      if (prev && prev !== s.label) {
        remaps.push({
          ...
          reason: "taxonomy sub label sync",
        });
      }
    }
  }
