...
//   4. 确定性 remap 直接改写 ai_tags；reclassify 写进待确认队列（不自动跑）
//   5. 版本化快照，便于审计/回滚
//
...
export const DEFAULT_TAXONOMY_POLICY = {
  ...
  autoReclassify: false, // 破坏性（需重读）重分类是否自动放行；默认否——要人工确认
...
export function buildRevisionUserPayload(app, signals, { evidencePerSignal = 6 } = {}) {
  ...
  for (const c of app.seed_categories ?? []) {
    ...
    lines.push(`- ${c.key}(${c.label}) 命中${cc?.count ?? 0}　子：${subs.join("、") || "（无）"}`);
  }
...
export function sanitizeTaxonomy(rawCategories) {
  ...
  for (const c of rawCategories) {
    ...
    out.push({ key, label: String(c.label).trim(), subcategories });
  }
  return out;
}
...
async function runBootstrap({ supabase, app, callModel, logger, dryRun }) {
  ...
  const taxonomy = sanitizeTaxonomy(parsed?.categories);
  ...
  await writeTaxonomy(supabase, app, taxonomy, meta);
...
async function runRevision({ supabase, app, callModel, logger, dryRun, signals, policy }) {
  ...
  const taxonomy = sanitizeTaxonomy(parsed?.taxonomy);
  if (!taxonomy.length) throw new Error("AI 修订返回的 taxonomy 为空");
  const { remaps, reclassifyChanges, affectedKeys } = partitionChanges(parsed?.changes);
  ...
  // 4a. 非破坏性：写新体系 + 版本快照 + 确定性改写 ai_tags
  await writeTaxonomy(supabase, app, taxonomy, meta);
...
async function writeTaxonomy(supabase, app, taxonomy, meta) {
  const { error } = await supabase.from("apps").update({ seed_categories: taxonomy, taxonomy_meta: meta }).eq("id", app.id);
  if (error) throw error;
}
...
async function applyRemapsToReviews({ supabase, app, remaps, logger }) {
  ...
  for (const r of reviews) {
    ...
    if (changed) updates.push({ id: r.id, ai_tags: tags, ai_tag_keys: tags.map((t) => t.key) });
  }
...
async function maybeQueueOrRunReclassify({ supabase, app, pending, policy, logger, dryRun, scope }) {
  ...
  logger.log(`[taxonomy] autoReclassify 开启：已重置 ${reset} 条评论的分类，下轮 cron 重读`);
