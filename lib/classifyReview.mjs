...
import {
  ...
  buildCauseConsequenceCalibratePrompt,
  buildClassifyPrompt,
  buildParentKeysWithSubs,
  fallbackTagsForRating,
  finalizeClassifiedTags,
  needsCauseConsequenceCalibration,
...
/** P1b：多具体类并存时的原因-后果专检（LLM 语义判断，无领域硬编码） */
export async function calibrateCauseConsequenceTags(apiKey, content, rating, tags, opts) {
  const systemPrompt = buildCauseConsequenceCalibratePrompt(opts);
  const tagJson = JSON.stringify(tags, null, 0);
  const parsed = await chatJson(
    apiKey,
    systemPrompt,
...
export async function classifyReviewWithPipeline(apiKey, { content, rating, knownTopLevelKeys, calibrate = true, ...promptOpts }) {
  ...
  const finalizeOpts = {
    ...
    rating,
    parentKeysWithSubs,
    seedCategories: promptOpts.seedCategories,
    universalSubcategories: promptOpts.universalSubcategories,
  };
  ...
  if (calibrate && needsCauseConsequenceCalibration(tags)) {
    const cc = await calibrateCauseConsequenceTags(apiKey, content, rating, tags, promptOpts);
    const finalized = finalizeClassifiedTags(cc.tags, finalizeOpts);
    if (passesValidation(finalized, knownTopLevelKeys, parentKeysWithSubs)) {
      tags = finalized;
      if (cc.calibrated) {
        calibrated = true;
        reason = cc.reason ?? reason;
      }
      validationOk = true;
    }
  }
  ...
  return {
    tags,
    calibrated,
    ...(reason ? { reason } : validationOk ? {} : { reason: "结构校验未通过" }),
  };
}
