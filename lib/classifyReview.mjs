...
export async function classifyReviewWithPipeline(apiKey, { content, rating, knownTopLevelKeys, calibrate = true, ...promptOpts }) {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    if (validateClassifiedTags(candidate, knownTopLevelKeys).ok) {
      tags = candidate;
      break;
    }
  }
  if (!tags.length) {
    tags = finalizeClassifiedTags(lastRaw, { knownTopLevelKeys, rating });
  }
  ...
  const finalized = finalizeClassifiedTags(result.tags, { knownTopLevelKeys, rating });
  if (!validateClassifiedTags(finalized, knownTopLevelKeys).ok) {
