...
export function scopedReviewSummaryCacheKey(appId, locale, since, until, tag, subTag, q, rating) {
  return `reviewScope|v4|${appId}|${locale ?? ""}|${since ?? ""}|${until ?? ""}|${tag ?? ""}|${subTag ?? ""}|${q ?? ""}|${rating ?? ""}`;
}
...
export function invalidateScopedReviewSummaryCache(appId) {
  ...
  const prefix = `reviewScope|v4|${appId}|`;
...
/** map-reduce 分批并行上限，避免瞬时打满 DeepSeek 限流 */
const MAP_REDUCE_CONCURRENCY = 8;

async function mapPool(items, fn, concurrency = MAP_REDUCE_CONCURRENCY) {
  const results = new Array(items.length);
  for (let i = 0; i < items.length; i += concurrency) {
    const slice = items.slice(i, i + concurrency);
    const chunkResults = await Promise.all(slice.map((item, j) => fn(item, i + j)));
...
export async function mapReduceSummarizeEvidence({ apiKey, scopeLabel, evidences, appContext }) {
  ...
  const partials = await mapPool(batches, (batch, i) =>
