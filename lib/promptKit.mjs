 ...
 * 优先 ai_tag_keys（列表筛选用），缺省回退 ai_tags。
 */
export function topLevelTagKeysForReview(review) {
  const fromKeys = review?.ai_tag_keys;
  // 字段存在（含空数组）即为准；仅缺字段时回退 ai_tags，与 applyReviewFilters 的 ai_tag_keys 筛选一致
  if (Array.isArray(fromKeys)) {
    return [...new Set(fromKeys.filter(Boolean))];
  }
  return [...new Set((review?.ai_tags ?? []).map((t) => t?.key).filter(Boolean))];
}
 ...
 * Demo / Ask 展示计数走 countReviewsMatching（与评论列表同口径），子之和不必等于母。
 */
...
export function buildAskPrompt({ appContext, timeRangeLabel, latestReviewDate, useEmoji = false, displayName, terminologyGlossary }) {
  return [
    ...
    "summarize_reviews 条数纪律：total 是与 Demo 列表/count_reviews 一致的评论总条数——回答「有多少/共几条」时只用 total。evidenceUsed 是纳入主题归纳的条数；excludedNoText 是无正文未纳入；notSummarized 是因单次上限未扫描的条数（不是无正文）。禁止把 evidenceUsed+excludedNoText 或 evidenceUsed+notSummarized 写成 total（错误示例：500+671=1171）；正确示例：「共 1197 条，其中 500 条纳入本次主题归纳，697 条因上限未扫描」。必须传达 countDisclaimer。",
