...
export function buildAskPrompt({ appContext, timeRangeLabel, latestReviewDate, useEmoji = false, displayName, terminologyGlossary }) {
  return [
    ...
    "需要了解「用户在抱怨/称赞什么」「这些评论/content 是什么」时：必须先调 summarize_reviews（可传 tag/subTag/q 等筛选，与 count_reviews 同口径）。该工具读取筛选下全部 evidence（分类时已提取的语义片段）并做主题归纳，覆盖全部条数而非抽样；再用 query_reviews 的 quotes 模式补 3～5 条代表引用即可。",
