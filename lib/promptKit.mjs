...
/** 跨顶层近义/同文案子问题：返回须消歧的 parentKey\\0subKey 集合 */
export function collectCrossCategorySubLabelCollisions(seedCategories = []) {
  const collisionKeys = new Set();
  for (const o of findCrossCategorySubOverlaps(seedCategories)) {
    collisionKeys.add(`${o.parentA}\0${o.subA}`);
    collisionKeys.add(`${o.parentB}\0${o.subB}`);
  }
  const byNorm = new Map();
  for (const cat of seedCategories ?? []) {
    for (const sub of cat.subcategories ?? []) {
      const norm = normalizeComparableText(sub.label);
      if (!norm) continue;
      const arr = byNorm.get(norm) ?? [];
      arr.push({ parentKey: cat.key, subKey: sub.key });
      byNorm.set(norm, arr);
    }
  }
  for (const arr of byNorm.values()) {
    const parents = new Set(arr.map((x) => x.parentKey));
    if (parents.size < 2) continue;
    for (const e of arr) collisionKeys.add(`${e.parentKey}\0${e.subKey}`);
  }
  return collisionKeys;
}

/**
 * taxonomy 写库前：跨父类同名/近义 sub label 自动加「（父类）」后缀，避免全链路歧义。
 * 与 findCrossCategorySubOverlaps 信号配合——能合并的仍靠修订合并，无法合并的用展示消歧。
 */
export function disambiguateCrossCategorySubLabels(seedCategories = []) {
  const collisionKeys = collectCrossCategorySubLabelCollisions(seedCategories);
  if (!collisionKeys.size) return seedCategories ?? [];

  return (seedCategories ?? []).map((cat) => ({
    ...cat,
    subcategories: (cat.subcategories ?? []).map((sub) => {
      const id = `${cat.key}\0${sub.key}`;
      if (!collisionKeys.has(id)) return sub;
      const suffix = `（${cat.label}）`;
      if (sub.label.includes(suffix)) return sub;
      return { ...sub, label: `${sub.label}${suffix}` };
    }),
  }));
}

/** 分类收尾：subLabel 与 taxonomy 设计稿对齐（含消歧后缀） */
...
export function buildClassifyPrompt({ appContext, seedCategories = [], universalSubcategories = {}, existingCustomTags = [], existingSubTags = {} }) {
  ...
  return [
    ...
    "子问题纪律：① 有子问题清单时，须优先从清单或下方「已确认子问题」里复用已有 subKey/subLabel。② 禁止为同一概念造近义 subKey。③ 现有子问题都无法覆盖、且该诉求不像会反复出现的稳定主题时，必须使用 subKey=general、subLabel=其他，禁止为单条或极少数评论新建 subKey。④ 禁止创建「××改进」「××优化」「综合诉求」等 meta 宽桶 sub，这类一律用 general(其他)。⑤ 只有预期会稳定复现的新主题才新建 subKey（英文 snake_case + 中文 label）。⑥ 禁止在不同顶层类下使用完全相同的 subLabel；若语义确实不同，须在 label 中加父类限定。",
...
export const SUBCATEGORY_DESIGN_POLICY = [
  ...
  "7. 跨顶层不重复：同一底层主题（如暗系统/ELO、人机队友、bug）只能在一个顶层类下出现；若两个顶层类的 sub 近义或同主题，修订时必须合并到 intent 更匹配的那一类并 remap/reclassify。若确属不同语义、无法合并，sub label 必须加父类限定（如「评分不合理（游戏平衡性）」），禁止跨父类完全相同的裸 label。",
