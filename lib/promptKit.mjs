...
export function buildClassifyPrompt({ appContext, seedCategories = [], existingCustomTags = [], existingSubTags = {} }) {
  ...
  return [
    ...
    "每个命中的类型，如果能进一步细分出具体子问题（比如「功能请求」下具体是要「阿拉伯语支持」还是「图片支持」；「功能故障」下具体是「保存失败」还是「闪退」），就再给一对 subKey/subLabel，格式跟主类型一样（subKey 用英文snake_case，subLabel 用简短中文）。如果这条评论在这个类型下已经足够具体、没有更细的子分类，subKey/subLabel 留 null，不要为了填而硬造一个。",
    "重要：subKey 不能跟常见类型、已有自定义类型撞名——如果你想用的子问题名字其实就是上面列出的某个顶层类型（比如想给「功能故障」标个 subKey 叫 performance_issue，但 performance_issue 本身已经是个顶层自定义类型），说明这条评论应该直接命中那个顶层类型本身，把它作为一个独立的顶层标签加进 tags 数组，不要把它塞进别的标签下面当子问题。",
    subTagLines.length
