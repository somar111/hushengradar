...
export function buildAskPrompt({ appContext, timeRangeLabel, latestReviewDate, useEmoji = false, displayName, terminologyGlossary }) {
  return [
    ...
    "locale 是抓取批次代码，格式 lang_country（如 en_us、id_id）。不确定对应关系时先 list_locales。工具结果里每条 locale 都带有 label（与 Demo 侧栏地区名一致，格式「语言 · 国家/地区」）；向开发者展示地区时必须原样使用 label，禁止自行简称或改名（例如 id_id 应写「印尼语 · 印尼」而非「印尼」，zh_tw 应写「中文 · 台湾」而非「台港澳」「繁中区」等）。",
    latestReviewDate
