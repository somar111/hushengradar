...
/** 分类收尾：subLabel 与 taxonomy 设计稿对齐 */
export function syncSubLabelsFromCatalog(tags, seedCategories = [], universalSubcategories = {}) {
  const catalog = buildCategoryCatalog(seedCategories, universalSubcategories);
  return (tags ?? []).map((t) => {
    if (!t?.subKey) return t;
    const parent = catalog.find((c) => c.key === t.key);
    const sub = parent?.subcategories?.find((s) => sanitizeTagKey(s.key) === sanitizeTagKey(t.subKey));
    if (!sub?.label) return t;
    return { ...t, subLabel: sub.label };
  });
}
...
/** Top 反馈 chip：单个子问题至少命中几条才单独展示（「其他」不受此限，永远沉底） */
export const MIN_SUBTAG_HIT_COUNT_FOR_CHIP = MIN_MEANINGFUL_SUBTAGS_FOR_BREAKDOWN;

/** 「其他」桶 enrich：该父类下 general 命中 ≥ 此值且过冷却期才触发 enrich→reclassify */
export const GENERAL_ENRICH_MIN_COUNT = 20;

/** 同一父类两次 general enrich 最短间隔（天） */
export const GENERAL_ENRICH_COOLDOWN_DAYS = 3;

export function isCatchAllSubKey(subKey, label) {
...
/** 聚合 subTags 里非 catch-all、且命中 ≥ MIN_SUBTAG_HIT_COUNT_FOR_CHIP 的子问题个数 */
export function countMeaningfulSubTags(subTags) {
  return Object.entries(subTags ?? {}).filter(
    ([key, v]) => !isCatchAllSubKey(key, v?.label) && (v?.count ?? 0) >= MIN_SUBTAG_HIT_COUNT_FOR_CHIP,
  ).length;
}

/** Top 反馈等聚合展示：须 ≥2 个「命中够多」的有效子问题才有 chip breakdown（general/其他 不计入） */
export function hasSubTagBreakdown(subTags) {
  return countMeaningfulSubTags(subTags) >= MIN_MEANINGFUL_SUBTAGS_FOR_BREAKDOWN;
}

/** 展示用：只保留命中 ≥ 门槛的 chip，「其他」始终保留 */
export function filterSubTagsForChipDisplay(subTags) {
...
/**
 * 子问题 chip label：去掉尾部括号内举例（如…、…等），只保留简短主题名。
 * taxonomy 归纳、分类 subLabel、ingest 门禁共用。
 */
export function normalizeSubcategoryLabel(label) {
...
export function parseClassifyTagsFromModel(parsedTags) {
  ...
  return parsedTags
    ...
    .map((t) => ({
      ...
      subLabel: t.subLabel ? normalizeSubcategoryLabel(t.subLabel) : null,
    }));
}
 ...
 * @param {{ knownTopLevelKeys?: Set<string>, rating?: number, parentKeysWithSubs?: Set<string>, seedCategories?: object[], universalSubcategories?: Record<string, {key:string,label:string}[]> }} [opts]
 * @returns {ClassifiedTag[]}
 */
export function finalizeClassifiedTags(rawTags, {
  knownTopLevelKeys,
  rating,
  parentKeysWithSubs,
  seedCategories,
  ...
  tags = syncSubLabelsFromCatalog(tags, seedCategories, universalSubcategories);
...
/** 从 ai_tags 提取去重顶层 key（分类结果的唯一来源；ai_tag_keys 仅为索引副本）。 */
export function aiTagKeysFromTags(aiTags) {
  return [...new Set((aiTags ?? []).map((t) => t?.key).filter(Boolean))];
}

/**
 * 与 count_reviews / Demo 评论列表同口径：每条评论每个顶层 tag 最多计 1 次。
 * 一律以 ai_tags 为准（与 applyReviewFilters 的 ai_tags @> [{key}] 筛选一致）。
 */
export function topLevelTagKeysForReview(review) {
  return aiTagKeysFromTags(review?.ai_tags);
}

/**
 * @param {Record<string, { label: string, count: number, summary?: string|null, repliedCount?: number, subTags: Record<string, { label: string, count: number }> }>} tagCounts
 * @param {{ tagSamples?: Record<string, string[]>, countOfficialReply?: boolean, skipCounts?: boolean }} [opts]
 */
export function accumulateTagCountsFromReview(review, tagCounts, opts = {}) {
  const tags = review.ai_tags ?? [];
  const skipCounts = opts.skipCounts === true;
  for (const key of topLevelTagKeysForReview(review)) {
    const matches = tags.filter((x) => x?.key === key);
    const t = matches[0] ?? { key, label: key };
    const entry = tagCounts[key] ?? {
      label: t.label || key,
      count: 0,
      summary: null,
      repliedCount: 0,
    ...
    if (!skipCounts) entry.count++;
    if (opts.countOfficialReply && review.official_reply) entry.repliedCount++;
    if (opts.tagSamples) {
      for (const m of matches.length ? matches : [t]) {
        const text = String(m.evidence ?? "").trim() || String(review.content ?? "").trim();
        if (text) {
          const list = opts.tagSamples[key] ?? [];
          list.push(text);
          opts.tagSamples[key] = list;
          break;
        }
      }
    }
    for (const m of matches) {
      if (m.subKey) {
        const sub = entry.subTags[m.subKey] ?? { label: m.subLabel || m.subKey, count: 0 };
        if (!skipCounts) sub.count++;
        entry.subTags[m.subKey] = sub;
      }
...
/** 从评论列表构建 tagCounts（cron 分类质量审计用：ai_tags 迭代口径，用于规则 3 恒等式校验） */
...
/**
 * 规则 3（仅 cron 分类质量审计，非 UI 展示）：有子问题清单的顶层，ai_tags 迭代下子 count 之和应等于母 count。
 * Demo / Ask 展示计数走 countReviewsMatching（与评论列表同口径，均以 ai_tags 为准），子之和不必等于母。
 */
...
/**
 * @deprecated 实现已迁至 subTagIngestGuards.mjs
 */
export { mergeObservedSubTagsIntoTaxonomy } from "./subTagIngestGuards.mjs";
...
export function buildClassifyPrompt({ appContext, seedCategories = [], universalSubcategories = {}, existingCustomTags = [], existingSubTags = {} }) {
  ...
  return [
    ...
    "子问题纪律：① 有子问题清单时，须优先从清单或下方「已确认子问题」里复用已有 subKey/subLabel。② 禁止为同一概念造近义 subKey。③ 现有子问题都无法覆盖、且该诉求不像会反复出现的稳定主题时，必须使用 subKey=general、subLabel=其他，禁止为单条或极少数评论新建 subKey。④ 禁止创建「××改进」「××优化」「综合诉求」等 meta 宽桶 sub，这类一律用 general(其他)。⑤ 只有预期会稳定复现的新主题才新建 subKey（英文 snake_case + 中文 label，简短主题名、禁止括号内举例）。⑥ 禁止在不同顶层类下使用完全相同的 subLabel；若语义确实不同，用不同措辞区分，禁止用括号加父类或举例。",
...
export const SUBCATEGORY_DESIGN_POLICY = [
  ...
  "9. 禁括号举例：label 只用 4～14 字简短主题名；禁止在 label 内用括号列举示例（如「（如…）」「（…等）」）。具体例子只写在 evidence，不进 chip。",
].join("\n");
...
export function buildTaxonomyDesignCalibratePrompt({ mode = "subcategories", parentKey, parentLabel } = {}) {
  ...
  return [
    ...
    "4. 去掉 label 尾部括号内举例（如…、…等）；chip 只展示简短主题名。",
    "5. 若草稿已合格，verdict 填 ok，原样返回；否则 verdict 填 revise，输出修订后的完整列表（不是增量 patch）。",
    mode === "full_taxonomy"
...
export function buildFeatureRequestSubsPrompt() {
  return [
    ...
    SUBCATEGORY_DESIGN_POLICY,
    COMPLAINT_VS_REQUEST_BOUNDARY,
    "每个 sub 应表达用户想要的新增对象（label 示例：「新增阿拉伯语」「新增某角色」），不要用笼统「改进/优化」类 label；禁止在 label 括号内枚举功能名。",
    '只输出 JSON：{"subcategories":[{"key":"...","label":"..."}]}',
  ].join("\n");
}

/**
 * 从「其他」桶样本归纳新 sub（已有清单只作排除，merge 写回 taxonomy）。
 */
export function buildGeneralBucketSubsPrompt({ parentLabel, existingSubs = [] }) {
  const existingBlock = existingSubs.length
    ? `已有子问题清单（禁止重复或近义，只补充清单未覆盖的新主题）：\n${existingSubs.map((s) => `- ${s.key}(${s.label})`).join("\n")}`
    : "当前尚无子问题清单。";
  return [
    `你是评论分析专家。以下样本均属「${parentLabel}」下的兜底桶「其他」——尚未归入具体子问题。`,
    existingBlock,
    "从样本中找出反复出现、且已有清单无法覆盖的稳定新主题，归纳 1~4 个新 subKey（英文 snake_case + 中文 label）。",
    "若样本全是零散一次性诉求、没有可复现主题，返回空数组 subcategories: []。",
    SUBCATEGORY_DESIGN_POLICY,
    COMPLAINT_VS_REQUEST_BOUNDARY,
    '只输出 JSON：{"subcategories":[{"key":"...","label":"..."}]}',
  ].join("\n");
...
const REPLY_CONTACT_POLICY = [
  ...
  "4. 不要承诺退款结果、到账时间或具体政策细节；给出授权邮箱只是引导人工处理，不是承诺一定退款成功。",
  "5. 开发者自定义联系方式里若写了「仅在某种情况下才提供联系方式」：评论涉及退款/退钱/要回钱、扣费或乱扣费、试用自动转订阅/续费、未经授权扣款、充值未到账、账号/订单需人工核实时，视为命中，必须在回复中写出匹配的授权邮箱。",
  "6. 命中联系方式规则时，「已转交开发团队跟进」不能替代授权邮箱——应先致歉/说明已收到，再给出对应邮箱引导用户联系处理。",
  "7. 仅当评论是一般功能体验差评、或与人工核实/退款/扣费无关的 bug 反馈时，才不附联系方式。拿不准是否命中时，若评论里已出现退款/扣费/订阅/账号等诉求，优先视为命中并给出对应邮箱。",
].join("\n");

/** 回复建议专用语气：在 TONE_POLICY 基础上，避免「转交开发团队」压过已授权联系方式 */
const REPLY_TONE_POLICY = [
  "语气克制、就事论事，站在帮助的立场，不替开发者下判断、不给优先级建议。",
  "注重帮助开发者解决问题，但不要说教，不要用训诫式语气（如「你应该/必须立刻」这类表达）。",
  "不要过度推断：只能基于已给数据说结论；若证据不足，用「可能/疑似」表达不确定性。",
  "不要编造你不确定的具体事实——比如具体的客服邮箱、网址、退款到账时间、政策细节等，不要做无法保证的承诺，比如保证退款一定成功、保证具体修复时间。",
  "未配置授权联系方式、或评论明显与退款/扣费/账号人工处理无关时，可用「已收到反馈，已转达给开发团队跟进」。若开发者已授权且评论命中其联系方式规则，必须提供对应授权邮箱，不要用「转交开发团队」代替人工入口。",
].join("\n");

const REFUND_CONTACT_RE =
  /refund|money\s*back|get\s+my\s+money|return\s+it|charge\s?back|want\s+my\s+money|退款|退钱|还钱|退还|要回.*钱|拿回.*钱/i;
const BILLING_ACCOUNT_CONTACT_RE =
  /charg(e|ed|ing)|bill(ing)?|deduct|subscription|subscribe|trial|renew|auto.?renew|试用|订阅|续费|扣费|乱扣|未经授权|unexpected\s+charge|without\s+(notice|inform|notif)|充值|未到账|account|封号|申诉|登录.*(丢|失)|lost\s+account|langganan|berlangganan|biaya|pembayaran/i;

/**
 * 评论是否明显命中「应提供授权联系方式」的场景（供 generateReplySuggestion 追加硬性提示）。
 * @param {string} content
 * @param {Array<{ label?: string, subLabel?: string, key?: string, subKey?: string }>} [tags]
 * @returns {"refund"|"billing_or_account"|null}
 */
export function detectReplyContactTrigger(content, tags = []) {
  const combined = [
    content ?? "",
    ...tags.map((t) => [t.label, t.subLabel, t.key, t.subKey].filter(Boolean).join(" ")),
  ].join("\n");
  if (REFUND_CONTACT_RE.test(combined)) return "refund";
  if (BILLING_ACCOUNT_CONTACT_RE.test(combined)) return "billing_or_account";
  return null;
}

export function buildReplyPrompt({ appContext, replyContext, displayName, terminologyGlossary, compact = false }) {
  ...
  const appCtx = appContext ? String(appContext).trim() : "";

  if (compact) {
    return [
      "你是 App 商店评论回复助手。用评论原文语言写 2~4 句（约 40~120 字），具体回应问题，有同理心，不要客服模板腔。",
    "有评论者昵称时回复须带上其昵称；问候等开场按评论语言与语境灵活写。无昵称则中性礼貌开场。禁止兄弟/bang/bro/亲等俚语。",
      "整条回复只用一种语言，禁止夹杂其他语言词（尤其不要把中文混进外语句）。",
      customContact
        ? `联系方式（仅按其中条件使用，禁止编造其他邮箱/网址/电话）：${customContact}`
        : "未配置联系方式——回复中禁止出现邮箱、网址、电话。",
      customTone ? `语气：${customTone}` : "",
      customStyle ? `句式：${customStyle}` : "",
      glossary,
      appCtx ? `App 背景（仅供理解产品）：${appCtx.slice(0, 600)}` : "",
      "直接输出回复正文，不要 JSON、不要前后缀。",
    ].filter(Boolean).join("\n");
  }
...
export function buildSummaryPrompt({ tagLabel, appContext }) {
  return [
    ...
    "优先写用户对具体功能/内容/机制的真实不满或诉求；不要把退游、退款、泄愤、差评等后果或笼统情绪写进短语。",
...
const SCOPED_REVIEW_INTENT_POLICY = [
  "归纳纪律（最高优先级）：",
  "1. 只归纳用户对具体功能/内容/机制/对象的真实不满或诉求（真实意图）；",
  "2. 不要把情绪后果（想退游、后悔、泄愤、想退款、一星差评）单独列为 theme；",
  "3. 顺带提及、且未在样本中反复出现的，不要单独列为 theme；",
  "4. 不要编造样本里没有的细节；只摆现象，不给改进建议。",
].join("\n");

/**
 * Ask summarize_reviews：对筛选 scope 内全部 evidence 做主题归纳（含 map-reduce 合并阶段）。
 * @param {{ scopeLabel: string, appContext?: string | null, phase?: "summarize"|"merge" }} opts
 */
export function buildScopedReviewSummaryPrompt({ scopeLabel, appContext, phase = "summarize" }) {
  if (phase === "merge") {
    return [
      `下面是对「${scopeLabel}」多批评论 evidence 的分批主题归纳，请合并为不重复的主题列表。`,
      SCOPED_REVIEW_INTENT_POLICY,
      '只输出 JSON：{"themes":[{"label":"简短主题名","description":"1-2句说明具体抱怨点/诉求对象"}]}，不要输出其他文字。',
      appContext ? `这款 App 的背景信息：${appContext}` : "",
    ].filter(Boolean).join("\n\n");
  }
  return [
    `下面是一批筛选为「${scopeLabel}」的评论 evidence——每条已是该评论里跟此筛选相关的片段（非完整原文）。`,
    "请按 theme 分组归纳这些 evidence 的具体抱怨点/诉求对象。",
    SCOPED_REVIEW_INTENT_POLICY,
    '只输出 JSON：{"themes":[{"label":"简短主题名","description":"1-2句说明"}]}，不要输出其他文字。',
    appContext ? `这款 App 的背景信息：${appContext}` : "",
  ].filter(Boolean).join("\n\n");
}
 ...
 */
const ASK_DATA_BOUNDARY = [
  "数据边界（工具结果里有什么、没有什么；超出边界的一律不能说，即使用常识觉得「应该有」）：",
  "有的：评论条数、星级与分布、均分、标签/子标签计数、日期范围、版本统计、官方回复率、locale+label（地区展示名）、evidence 摘录、评论原文/译文（query_reviews full）、是否已有官方回复。",
  "没有的：点赞数、有用票、热度/排名、下载量、用户画像、商店互动量。不得用这些描述「代表性」——改用条数、占比或摘录。",
  "地区名：面向开发者的回答里只写工具返回的 label（如「英语 · 美国」「印尼语 · 印尼」），禁止出现 lang_country 抓取批次代码（如 en_us、id_id、zh_tw）——那是调工具用的内部参数，不是给用户看的术语。不要自行从代码推断改名（反例：id_id→「印尼」、zh_tw→「台港澳」）。介绍能力、举例说明时也用 label，不要举代码。",
  "联系方式：邮箱/网址/电话/客服入口仅当评论或官方回复原文逐字出现时可引用并标明出处；不得用训练数据补全。",
].join("\n");

const ASK_EVIDENCE_POLICY = [
  "证据纪律（最高优先级，覆盖其他写作习惯）：",
  "1. 你只能陈述：工具返回的统计数字、评论/evidence 原文里明确写出的信息、基于前两者的简单计算（占比/排序/计数）。",
  "2. 禁止把推测写成事实。无评论或统计直接支撑时，不能说用户身份、经济状况、心理动机、地区文化归因等，也不要用「多为/主要是/系统性」包装猜测。",
  "3. App 背景信息只用于理解产品功能，不能当作「这批评论用户是谁」的证据。",
  "4. 做比较/排名时说明依据（样本量、标签分布、摘录）；样本有限先说「基于当前 N 条」。",
  "5. 归纳用「在 X 条中有 Y 条提到…」「评论里反复出现…」；若只能推测，写「可能/疑似」并紧跟「依据：…」。",
  "6. 不要为了回答完整而补故事；证据不足直接说「当前数据不足以判断」。",
].join("\n");
...
function buildAskFormatPolicy(useEmoji) {
  ...
  return [
    ...
    "5. 引用非中文原文（evidence、评论摘录、用户原话）时：必须在同一处紧随其后给出简体中文译意，例如「（印尼语原文：…，译：…）」或「原文…——译：…」。工具结果有 translatedZh / full 模式 text 时优先用其中译文；没有则基于工具内容忠实翻译。禁止只抛原文不译。",
  ].join("\n");
}
...
export function buildAskPrompt({ appContext, timeRangeLabel, latestReviewDate, useEmoji = false, displayName, terminologyGlossary }) {
  return [
    ...
    ASK_DATA_BOUNDARY,
    ASK_EVIDENCE_POLICY,
    "问「某顶层标签/子标签有多少条评论」时：优先 count_reviews（tag + 必要时 subTag），以 total 为条数口径——与 Demo 列表一致。get_stats 的 tagBreakdown/subTagBreakdown 计数亦同口径，但 count_reviews 更直接。UI 子标签「其他」对应 subTag=general。",
    "问「共多少条评论 / 几星占比 / 均分 / 回复率」等聚合统计时：优先 get_stats。totalReviews 与 Demo 侧栏「全部 (N)」同口径（含无星级评论）；ratedReviewTotal 仅为 1~5 星之和——禁止把 ratedReviewTotal 或 ratingDistribution 各星 count 相加当作总条数。星级占比默认用 ratingDistributionPct（分母 totalReviews）；仅当用户明确问「有星级的评论里」才用 ratingDistributionPctOfRated。",
    "需要了解「用户在抱怨/称赞什么」「这些评论/content 是什么」时：必须先调 summarize_reviews（可传 tag/subTag/q 等筛选，与 count_reviews 同口径）。该工具对筛选下全部有内容的 evidence 做主题归纳（非 query_reviews 抽样）；再用 query_reviews 的 quotes 模式补 3～5 条代表引用即可。",
    "summarize_reviews 条数纪律：total 是与 Demo 列表/count_reviews 一致的评论总条数——回答「有多少/共几条」时只用 total。evidenceUsed 是纳入主题归纳的条数（≤1600）；excludedNoText 是无正文未纳入；notSummarized 是超出 1600 上限未抽样的条数。禁止把 evidenceUsed+excludedNoText 写成 total。必须传达 countDisclaimer。",
    "禁止仅用 query_reviews 的少量样本归纳主题——query_reviews 只用于代表引用、关键词 existence 检查或用户明确要求逐条原文。",
    "query_reviews：mode=quotes（默认，evidence+元数据+translatedZh 若有；limit 默认 5）；mode=full 才含译文/原文全文。支持 page 翻页；用户要逐条看全部原文时提示去 Demo「评论查看&回复」或多次翻页，不要假装已读完全部。",
    "按关键词找评论（如『有没有评论提到印度/某功能/某词』『有评论要求新英雄吗』）用 query_reviews 的 q 参数——它模糊匹配评论原文与中英文翻译。存在性问题优先单次 query_reviews（q=核心词、since 尽量放宽），以 total 判断有没有，补 2～3 条例证后即作答，勿连环调多个工具。",
    "locale 是调工具时的抓取批次参数（lang_country，如 en_us），仅出现在 tool 调用里；回答正文、能力介绍、举例一律用 list_locales / 工具结果里的 label，禁止把 locale 代码写给开发者。不确定对应关系时先 list_locales。",
    latestReviewDate
    ...
    "目标是帮助开发者定位问题：可执行建议仅限产品/版本/功能/运营方向（如排查某版本崩溃、优化订阅说明），不要写具体联系方式或外链（见数据边界）。",
    "禁止使用内部术语（如「子问题」「taxonomy」「聚类」、lang_country 地区代码）；请改成开发者可直接理解的自然语言。",
    "不要输出 JSON。",
  ].filter(Boolean).join("\n\n");
}
