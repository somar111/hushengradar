 ...
 * @param {{ seedCategories?: object[], universalSubcategories?: Record<string, {key:string,label:string}[]> }} opts
 */
export function needsSemanticCalibration(tags, { seedCategories = [], universalSubcategories = {} } = {}) {
  ...
  for (const t of tags ?? []) {
    if (t.key === "feature_request") continue;
    ...
    // 子问题不在 taxonomy 设计清单且非 general → 可能临场造 tag / 归错父类
...
export function buildClassifyPrompt({ appContext, seedCategories = [], universalSubcategories = {}, existingCustomTags = [], existingSubTags = {} }) {
  ...
  return [
    ...
    "分类体系：\n" + taxonomyLines.join("\n"),
    "跨类边界（最高优先级之一）：",
    "- feature_request(功能请求)：用户想要新增/扩展能力或内容（如希望出新英雄、请加某语言、想要新地图）。这是诉求，不是报现有功能坏了。",
    "- 各 App 专属「问题/抱怨」类（如内容与功能、网络与性能）：描述的是现有功能/内容的缺陷、不足、体验差——不是单纯想要新东西。",
    "- 同一条评论可以同时命中「抱怨现有问题」和「功能请求」，但不要把明显的功能诉求塞进抱怨类的 subKey 里；功能诉求应单独命中 feature_request。",
    customList
...
export function buildCalibratePrompt(opts) {
  ...
  return [
    ...
    classifyBlock,
    "校准纪律：",
    "1. 若初步分类与评论意图、各类 intent 边界一致，verdict 填 ok，原样返回 tags。",
    "2. 若明显归错类（典型：功能诉求被塞进抱怨类如 content_features 的 subKey；或抱怨被标成 feature_request），verdict 填 reroute，给出修正后的完整 tags 数组。",
...
export function buildFeatureRequestSubsPrompt() {
  return [
    ...
    "只归纳「想要新增/改进」类诉求，不要把报 bug、体验差、退款等抱怨塞进来。",
...
export function buildTaxonomyRevisionPrompt() {
  return [
    ...
    "应当修订的典型信号：① 同一底层问题被拆成多个近义子问题（该合并）；② 大量、且跨时间持续地落在「孤儿顶层标签」或 vague_complaint，说明体系缺了某个真实类目（该新增/拆分）；③ 某子问题已稳定大量、跨时间出现却还没进体系（该固化）；④ 某 label 措辞不准或与母类重名（该改名）；⑤ **跨类 misroute**：某 App 专属母类下稳定出现语义属于 feature_request（或另一 universal 类）的子问题——应 remap 到正确顶层，或触发 reclassify。",
...
export function buildAskPrompt({ appContext, timeRangeLabel, latestReviewDate, useEmoji = false }) {
  return [
