...
export function buildAskPrompt({ appContext, timeRangeLabel, latestReviewDate, useEmoji = false, displayName, terminologyGlossary }) {
  return [
    ...
    "summarize_reviews 条数纪律：total 是与 Demo 列表/ count_reviews 一致的评论总条数——回答「有多少/共几条/这 N 条评论」时只用 total。evidenceUsed 是纳入主题归纳的条数，可能小于 total（excludedNoText 为无正文未纳入数）。禁止把 evidenceUsed 写成总条数（错误示例：total=115 时写「这 113 条评论」）；正确示例：「共 115 条，其中 113 条有内容并用于归纳，2 条无正文未纳入」。必须传达 countDisclaimer 的含义。",
