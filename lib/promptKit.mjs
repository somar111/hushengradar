...
export function buildClassifyPrompt({ appContext, seedCategories = [], existingCustomTags = [], existingSubTags = {} }) {
  ...
  return [
    ...
    "一条评论可以命中多个类型。每条评论至少要命中一个类型，不能是空数组——如果通篇只是笼统的负面情绪、找不到任何具体可定位的问题（比如只说「垃圾」「最烂的app」「一星」而没说哪里不好），就命中 vague_complaint(意义不明的纯抱怨)；只要评论里有任何具体的问题指向，就用对应的具体类型，不要用 vague_complaint。vague_complaint 命中时通常它就是唯一的标签，不要和具体类型并列。",
    "评论经常一次说好几件不相关的事（比如同时抱怨广告多、又抱怨文件丢失）——每个命中的类型都要单独给一句 evidence：只描述这条评论里跟这个类型相关的那部分内容，用简短中文转述，不要把评论里属于其他类型的内容也混进来。",
    "除了 vague_complaint 之外，每个命中的类型都必须再给一对 subKey/subLabel，指出这条评论在这个类型下具体是哪个子问题（比如「功能请求」下具体是「阿拉伯语支持」还是「图片支持」；「功能故障」下具体是「保存失败」还是「闪退」；「好评」下具体是夸「免费功能多」还是夸「界面好看」）。subKey 用英文snake_case，subLabel 用简短中文，命名要能复用——同一个子问题在不同评论里要用同一个 subKey，不要造一堆近义碎子问题。只有 vague_complaint 这个类型 subKey/subLabel 留 null。",
    "重要：subKey 不能跟常见类型、已有自定义类型撞名——如果你想用的子问题名字其实就是上面列出的某个顶层类型（比如想给「功能故障」标个 subKey 叫 performance_issue，但 performance_issue 本身已经是个顶层类型），说明这条评论应该直接命中那个顶层类型本身，把它作为一个独立的顶层标签加进 tags 数组，不要把它塞进别的标签下面当子问题。",
    subTagLines.length
      ? `这款App之前已经在某些类型下造过这些子问题，如果评论内容符合，优先复用，不要重复造近义的新 subKey：\n${subTagLines.join("\n")}`
      : "",
