...
/** 母/子 label 或 subKey 与母 key 明显同名/近名——应合并或改用兜底子问题 */
...
export function buildClassifyPrompt({ appContext, seedCategories = [], existingCustomTags = [], existingSubTags = {} }) {
  ...
  return [
    ...
    "除了 vague_complaint 和 praise，每个命中的类型都必须再给一对 subKey/subLabel，指明在这个类型下具体是哪个子问题。优先从上面该类型列出的子问题里选；只有都不合适时才新建一个 subKey（英文 snake_case + 中文 label，命名要能被后续评论复用）。vague_complaint 和 praise 的 subKey/subLabel 必须留 null。",
    ...
    subTagLines.length
      ? `以下子问题已在 taxonomy 或历史评论中确认，符合就优先复用：\n${subTagLines.join("\n")}`
      : "",
