...
export const CLASSIFY_CORE_PRINCIPLE = [
  ...
  "2. 注意区分评论里的原因和后果（原因导致的后果）——找出用户真正的不满对象，按不满对象归类。",
].join("\n");
...
/** 聚合展示：子问题是否构成有效拆分（须 2+ 个才有 breakdown 价值） */
export function hasSubTagBreakdown(subTags) {
  return Object.keys(subTags ?? {}).length > 1;
}
...
function fixParentChildNaming(tag, knownTopLevelKeys, parentKeysWithSubs) {
  ...
  if (!tag.subKey) return ensureSubTagOnTag(tag, parentKeysWithSubs);
  if (
    tag.subKey === tag.key
...
export function finalizeClassifiedTags(rawTags, { knownTopLevelKeys, rating, parentKeysWithSubs } = {}) {
  ...
  const validation = validateClassifiedTags(tags, known, subsKeys);
...
export function buildClassifyPrompt({ appContext, seedCategories = [], universalSubcategories = {}, existingCustomTags = [], existingSubTags = {} }) {
  ...
  return [
    ...
    "子问题纪律：① 有子问题清单时，须优先从清单或下方「已确认子问题」里复用已有 subKey/subLabel。② 禁止为同一概念造近义 subKey。③ 现有子问题都无法覆盖时，才新建 subKey（英文 snake_case + 中文 label）。",
