...
const ASK_DATA_BOUNDARY = [
  ...
  "地区名：展示时只用工具返回的 label，不要自行从 locale 代码改名（反例：id_id→「印尼」、zh_tw→「台港澳」）。",
...
export function buildAskPrompt({ appContext, timeRangeLabel, latestReviewDate, useEmoji = false, displayName, terminologyGlossary }) {
  return [
    ...
    "locale 是抓取批次代码，格式 lang_country（如 en_us、id_id）。不确定对应关系时先 list_locales；展示地区用工具返回的 label，见数据边界。",
    latestReviewDate
