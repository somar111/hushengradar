...
const REPLY_CONTACT_POLICY = [
  "联系方式纪律（覆盖一切客服话术习惯）：",
  "1. 回复中不得出现邮箱、网址、电话，除非「本条评论原文」或下方提供的「官方曾回复示例」里逐字出现。",
  "2. 没有官方回复示例时，不要引导用户联系客服邮箱/外链/订阅管理页；改用「我们已收到反馈，会转交团队跟进」这类表述。",
  "3. 禁止根据 App 品牌名或训练数据猜测 support@、feedback@ 等邮箱，禁止编造 wps.com 等链接。",
  "4. 不要承诺退款结果、到账时间或具体政策细节。",
].join("\n");

export function buildReplyPrompt({ appContext, officialReplyExample, replyContext }) {
  ...
  const customLines = [
    ...
    customContact ? `联系方式/引导用语（开发者已授权，可在回复中使用）：${customContact}` : "",
  ].filter(Boolean);
  ...
  return [
    ...
    officialReplyExample
      ? `官方曾回复示例（联系方式/处理流程只能引用这里、评论原文或开发者自定义联系方式里出现过的信息，不得自行补充）：${officialReplyExample}`
      : customContact
        ? "本条评论没有官方回复示例——联系方式只能使用开发者自定义里写明的信息，不得自行编造其他邮箱、网址或电话。"
        : "本条评论没有官方回复示例——回复中禁止出现任何邮箱、网址、电话或外链入口。",
