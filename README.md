...
| 阶段 | 做什么 | Demo 里 |
|---|---|---|
| **监控** | 按 locale 增量拉评论、去重入库 | 定时 cron + 水位线 |
| **理解** | 打标签、翻译、写标签摘要 | 离线 AI 管线 |
| **洞察** | 看分布、下钻子问题、按版本/地区/时间筛 | Top 反馈、统计图、insights |
| **互动** | 追问、起草回复 | 问 AI、单条回复建议、术语表 |

Demo 里互动主要是辅助决策（出回复草稿、对话分析）。以后接官方 API 可以直接回帖到应用商店。
